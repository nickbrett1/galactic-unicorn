...
default. Start from the committed example:

```bash
cp config_secrets.example.py config_secrets.py   # then fill in SSID + password
```
...
> **Better home for these later:** a deploy step could pull the credentials
> from Doppler (`doppler secrets get WIFI_SSID --plain`) and generate
> `config_secrets.py`, so the repo and the developer machine hold no copy. The
> board still needs a file, so `config_secrets.py` remains the on-device shape.
