...
# "galactic-unicorn-dev" (the repo name plus the "-dev" suffix).
...
# than adding a second one - a stale "galactic-unicorn-dev" is overwritten by
...
PROJECT_NAME="galactic-unicorn"
AGENT_NAME="galactic-unicorn-dev"
WORKSPACE="/workspaces/galactic-unicorn"
...
if [ -x "/workspaces/galactic-unicorn/scripts/agent-dev.sh" ]; then
  "/workspaces/galactic-unicorn/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
