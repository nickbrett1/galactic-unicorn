...
if [ -d dist ]; then
  tar -czf "$OUT_DIR/galactic-unicorn-any.tar.gz" -C dist .
  echo "packaged dist/ as galactic-unicorn-any.tar.gz"
fi
...
for target in ; do
  if [ -d "build/$target" ]; then
    ...
    tar -czf "$OUT_DIR/galactic-unicorn-$target.tar.gz" -C "build/$target" .
    echo "packaged build/$target/ as galactic-unicorn-$target.tar.gz"
  else
